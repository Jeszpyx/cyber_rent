 ...
 * Запуск: npm run simulate [-- <партий> <ботов>], по умолчанию 1000 партий по 4 игрока.
 */
...
for (let seed = 1; seed <= games; seed++) {
  let state = createGame({ playerName: 'Игрок', bots, seed });
  let turns = 0;
...
console.log(`Партий: ${games}, игроков: ${players}, лимит: ${MAX_TURNS} ходов (${((Date.now() - started) / 1000).toFixed(1)} с)`);
